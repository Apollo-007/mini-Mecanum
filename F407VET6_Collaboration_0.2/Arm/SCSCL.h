...
extern void RegWriteActionCL(void);//异步写位置执行
...
extern int EnableTorqueCL(uint8_t ID, uint8_t Enable);//扭矩控制指令
extern int unLockEpromCL(uint8_t ID);//eprom解锁
extern int LockEpromCL(uint8_t ID);//eprom加锁
extern int FeedBackCL(int ID);//反馈舵机信息
extern int ReadPosCL(int ID);//读位置
extern int ReadSpeedCL(int ID);//读速度
extern int ReadLoadCL(int ID);//读输出扭力
extern int ReadVoltageCL(int ID);//读电压
extern int ReadTemperCL(int ID);//读温度
extern int ReadMoveCL(int ID);//读移动状态
extern int ReadCurrentCL(int ID);//读电流
extern int getErrCL(void);//返回通信状态,0无出错,1通信出错
